...
## A generated page counted as new source

The code mirror keys on "content the state mirror does not carry", to avoid a
paste per bank while the push route is down. `autopilot/STATUS.md` sits outside
`state/` and so counted — and it is regenerated by every heartbeat, so the
mirror published a code paste every thirty minutes about source nobody had
touched. Exactly the flood the key exists to prevent, arriving through a
generated file.

The sharper question is not *"is it in the state mirror"* but **"could a fresh
