...
const body = m[1] + '\n;return {parseTape,buildNotes,notesToEvents,transitions,steerSegments,matchEdge,gradeOf,scoreOf,WIN,LANES,ROWS,NOTES,TRANS,RACE_SEGS,TAPE_START,TAPE_END,FINISH_MS,SECTIONS,RAW_TAPE};';
...
console.log('  lanes: left '+byLane(0).length+', gas '+byLane(1).length+', brake '+byLane(2).length+', right '+byLane(3).length);
const race = N.filter(n=>n.end>0);
ok(race.length===15,'15 notes touch the race (got '+race.length+')');
ok(N.filter(n=>n.lane===2 && n.start>=0).length===1,'exactly one brake note after the line');
...
ok(N.some(n=>n.lane===1 && n.open && n.start===-270),'gas is one open note from -0.270 to the end');
ok(N.some(n=>n.lane===3 && n.open && n.start===5230),'final right is open to the finish');
...
ok(evRace.every(e=>e.t>=-270),'race mode keeps only the carry-in and later');
ok(evRace.filter(e=>e.type==='press').length===evRace.filter(e=>e.type==='release').length+2,
   'two notes (gas, final right) are open-ended, so 2 more presses than releases');
// judging behaviour
...
ok(api.matchEdge(evs,2,'press',4000)===null,'a brake press at 4.000 matches nothing (spurious)');
