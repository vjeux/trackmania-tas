...
| | `ghost unwrap IN.Replay.Gbx OUT.Ghost.Gbx` | the ghost node of a replay as a standalone ghost: sliced byte-for-byte, telemetry node re-indexed 2 → 1 (the only difference, measured), control requires exactly that and the same tape/result/declared time back. Built for the client's `ProgramData\Trackmania\MediaTrackerCache\MTAuthorGhost<map name>.Ghost.gbx` — the run a player finished in the editor's test mode, kept by MAP NAME as the MediaTracker's "Author ghost"; a replay (map + one ghost) despite the extension |
| | `ghost lcp FILE.LaunchedCP.gbx [--json O] [--csv O] [--samples]` | the editor's launched-checkpoints cache (`ProgramData\Trackmania\LaunchedCheckpointsCache\<map name>.LaunchedCP.gbx`, class 0x03262000, written at every checkpoint crossing in test mode): per checkpoint reached, the full car state at the crossing (position, quaternion, velocity, angular velocity, wheel steer/rotation, signed forward speed) and the last ~1.5 s of approach as 116-byte vehicle samples with the inputs — the route skeleton of an UNFINISHED run. Landmark = the `tmmaps waypoints` index; times are the test-session clock |
...
### Three more files that crash the client on import, and the map that does not

**2026-08-24, 285885.** All three of that page's published tapes kill the game
process on `ImportGhosts`; the same map with the downloaded human recording, and
two other maps with their own published tapes, import in the same session. So
the crash is the FILE, and on this page it was the reason the map had no clip at
all.

| import | result |
|---|---|
| 285885 + the human 61.229 | scene ready |
| 267859 + `TAS_10758` (single-entity, has a clip) | scene ready |
| 284238 + `TAS_97325` | scene ready |
| 285885 + `TAS_50229` / `POKE_1input_50659` / `TRIGGERPOKE_50469` | **process gone** |

All three carry the V11 shape — no live non-vehicle record — and all three also
fail V6 (kappa 0.342 / 0.343 / 0.346: the recording is not the tape's run).
`ghost regen` followed by `ghost record graft-scene --from` the human recording
produced `TAS_50229_shootable.Ghost.Gbx`: V1–V11 clean, kappa 1.000, oracle
50.229 — **and it imports**, which is what finally filmed that map.

That is the second confirmed repair of this shape (173691 was the first), and
the counter-example still stands: `TAS_67319` and 284238's `TAS_97325` have no
live non-vehicle record either and import cleanly. **The shape is a suspect, not
a verdict** — but when an import kills the game, it is the first thing to try.

**A crash reads as `read: Connection reset by peer (os error 104)` in
`shootctl setup`.** That is indistinguishable, at the socket, from the plugin
being reloaded under you — so check the game's PID either side of the call
before concluding anything: `tasklist /FI "IMAGENAME eq Trackmania.exe"`.
