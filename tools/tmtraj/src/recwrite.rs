...
use crate::entrec::{Ent, RecordData, Res};
use crate::gbx::Gbx;
...
pub fn find_rec_site(body: &[u8]) -> Res<RecSite> {
    let needle = crate::entrec::CLASS_CPLUGENTRECORDDATA.to_le_bytes();
    let mut off = 0usize;
    loop {
        ...
        let skip = crate::gbx::all_skip_chunks(body)
            .into_iter()
    ...
    F: FnOnce(&mut RecordData) -> Res<()>,
{
    ...
    let mut rd = crate::entrec::parse_record_data(&blob, site.version)?;
    f(&mut rd)?;
