...
pub const FIELD_CONFIDENCE: &[FieldDoc] = &[
    ...
    FieldDoc { name: "side_speed", conf: Conf::Derived, note: "u16 at byte 2: ((v/65536)-0.5)*2000" },
...
mod tests {
    ...
    #[test]
    fn confidence_table_covers_every_sample_field() {
        // every documented field must be readable ...
        for f in FIELD_CONFIDENCE {
            let _ = Sample::default().field(f.name);
        }
        // ... and every CSV column must be documented
        for c in CSV_COLUMNS {
            assert!(
                FIELD_CONFIDENCE.iter().any(|f| f.name == *c),
                "CSV column {} missing from FIELD_CONFIDENCE",
                c
            );
        }
        assert_eq!(FIELD_CONFIDENCE.len(), 48);
        let v = FIELD_CONFIDENCE.iter().filter(|f| f.conf == Conf::Verified).count();
        let d = FIELD_CONFIDENCE.iter().filter(|f| f.conf == Conf::Derived).count();
        let g = FIELD_CONFIDENCE.iter().filter(|f| f.conf == Conf::Guess).count();
        assert_eq!((v, d, g), (18, 18, 12));
    }
}
