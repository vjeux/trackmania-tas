//! Local search for bit-exact translation per axis. Usage: tsearch HIS.ITEM SRCFILE
...
fn main() {
    ...
    // collect tight pairs (u, his) per axis
                ...
                for tt in idx.chunks(3) {
                    ...
                    if let Some(v) = mmap.get(&k) {
                        ...
                        for perm in [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]] {
                            ...
                            if ok {
                                for cc in 0..3 {
                                    pairs.push(([s[perm[cc]][0]*0.5, s[perm[cc]][1]*0.5, s[perm[cc]][2]*0.5], p[cc]));
                                }
                                break;
                            }
                        }
    ...
    // local search per axis: t in base ± 5 ulp (ulp at 8m = 2^-20)
    for ax in 0..3 {
        let ulp = 2f64.powi(-20);
        let mut best = (0usize, 0i32);
        for step in -8..=8 {
            let t = base[ax] + step as f64 * ulp;
            let mut hit = 0;
            for (u, h) in &pairs {
                if ((u[ax] + t) as f32).to_bits() == h[ax].to_bits() { hit += 1; }
            }
            if hit > best.0 { best = (hit, step); }
            println!("  ax{ax} step{step:+}: bitexact {hit}/{}", pairs.len());
        }
        println!("ax{ax}: best step={} hit={}", best.1, best.0);
    }
}
