//! Structure of X errors. Usage: xerr HIS.ITEM SRCFILE
...
fn main() {
    ...
    let t = [-0.001975040f64, -0.000007558, -0.023214310];
    // buckets of (src_x rounded to m) -> list of x errors in ulps
                    ...
                    if let Some(v) = mmap.get(&k) {
                        ...
                        for perm in [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]] {
                            ...
                            if ok {
                                for cc in 0..3 {
                                    tot += 1;
                                    let u = (s[perm[cc]][0]*0.5) as f32;
                                    let ex = (u as f64 + t[0]) as f32;
                                    let err_ulp = (p[cc][0] as f64 - ex as f64) / 2f64.powi(-20);
                                    buckets.entry((s[perm[cc]][0]).round() as i32).or_default().push(err_ulp);
                                }
                                break;
                            }
                        }
    ...
    for (sx, v) in &buckets {
        ...
        println!("src_x~{sx}: n={} err_ulp min={:.2} p50={:.2} max={:.2}", s.len(), s[0], s[s.len()/2], s[s.len()-1]);
    }
}
