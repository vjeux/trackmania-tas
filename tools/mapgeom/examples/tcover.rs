//! Max-overlap translation: find t covered by most per-pair intervals.
//! Usage: tcover HIS.ITEM SRCFILE
...
fn main() {
    ...
    // per-axis intervals from UNIQUE-source pairs only (skip ambiguous)
    let mut ivs: [Vec<(f64, f64)>; 3] = [Vec::new(), Vec::new(), Vec::new()];
                            ...
                            if ok {
                                for cc in 0..3 {
                                    for ax in 0..3 {
                                        ...
                                        let h = p[cc][ax] as f64;
                                        // ulp at the rounded value: use his value magnitude
                                        let e = (h.abs().max(1e-30) as f64).log2().floor() as i32;
                                        let ulp = 2f64.powi(e - 23);
                                        ivs[ax].push((h - u - ulp/2.0, h - u + ulp/2.0));
                                    }
                                }
                                break;
                            }
    ...
    for ax in 0..3 {
        ...
        let mut ev: Vec<(f64, i32)> = Vec::new();
        for (lo, hi) in &ivs[ax] {
            ev.push((*lo, 1));
            ev.push((*hi, -1));
        }
        ev.sort_by(|x, y| x.0.partial_cmp(&y.0).unwrap());
        let (mut cur, mut best, mut bestt) = (0, 0, 0.0);
        for (t, delta) in &ev {
            cur += delta;
            if cur > best { best = cur; bestt = *t; }
        }
        println!("ax{ax}: intervals={} max_overlap={best} ({:.2}%) at t={bestt:.10}", ivs[ax].len(), 100.0*best as f64/ivs[ax].len() as f64);
    }
}
