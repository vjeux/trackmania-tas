//! Are outlier verts in colliding-key tris? Usage: outlamb HIS.ITEM SRCFILE
...
fn main() {
    ...
    let t = [-0.001976013f32, -0.0000076293945312, -0.023214340];
    ...
    // colliding keys
    let mut coll: BTreeSet<[(i32, i32, i32); 3]> = BTreeSet::new();
    for (k, v) in &mmap {
        if v.len() > 1 { coll.insert(*k); }
    }
    let hd = std::fs::read(&a[1]).unwrap();
    let hf = mapgeom::static_item::file::parse_file(&hd).unwrap();
    let so = hf.item.static_object().unwrap();
    let s2 = so.solid2().unwrap();
    let (mut out_total, mut out_coll, mut in_total) = (0, 0, 0);
                ...
                for tt in idx.chunks(3) {
                    ...
                    if let Some(v) = mmap.get(&k) {
                        ...
                        for perm in [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]] {
                            ...
                            if ok {
                                for cc in 0..3 {
                                    // outlier if ANY axis misses under formula E
                                    let mut miss = false;
                                    for ax in 0..3 {
                                        if s[perm[cc]][ax]*0.5f32 + t[ax] != p[cc][ax] { miss = true; }
                                    }
                                    if miss {
                                        out_total += 1;
                                        if in_coll { out_coll += 1; }
                                    } else {
                                        in_total += 1;
                                    }
                                }
    ...
    println!("outlier_corners={out_total} of_which_in_colliding_tris={out_coll} inlier_corners={in_total}");
}
