//! Count mmkey collisions (tri-pairing ambiguity). Usage: keycollide HIS.ITEM SRCFILE
...
fn main() {
    ...
    let mut colltris = 0;
    for (_, c) in &mmap {
        if *c > 1 { coll += 1; colltris += c; }
    }
    println!("src tris={n} distinct_keys={} colliding_keys={coll} tris_in_collisions={colltris}", mmap.len());
    // same for his
    let hd = std::fs::read(&a[1]).unwrap();
    let hf = mapgeom::static_item::file::parse_file(&hd).unwrap();
    let so = hf.item.static_object().unwrap();
    let s2 = so.solid2().unwrap();
    let mut hmap: BTreeMap<[(i32, i32, i32); 3], usize> = BTreeMap::new();
    let mut hn = 0;
        ...
        if let Some(vref) = s2.visuals.get(vi) {
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                for tt in idx.chunks(3) {
                    ...
                    k.sort();
                    *hmap.entry(k).or_insert(0) += 1;
                }
            }
        }
    }
    let (mut hcoll, mut hcolltris) = (0, 0);
    for (_, c) in &hmap {
        if *c > 1 { hcoll += 1; hcolltris += c; }
    }
    println!("his tris={hn} distinct_keys={} colliding_keys={hcoll} tris_in_collisions={hcolltris}", hmap.len());
}
