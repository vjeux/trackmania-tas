//! Bit-level expected-vs-actual for one material. Usage: bitcmp HIS.ITEM SRCFILE SUBSTR
                            ...
                            if ok {
                                for cc in 0..3 {
                                    ...
                                    for ax in 0..3 {
                                        ...
                                        if ((u + t[ax]) as f32).to_bits() != p[cc][ax].to_bits() { miss = true; }
                                    }
                                    if miss && shown < 6 {
                                        shown += 1;
                                        println!("corner his=[{:.7},{:.7},{:.7}]", p[cc][0], p[cc][1], p[cc][2]);
                                        for ax in 0..3 {
                                            let u = s[perm[cc]][ax]*0.5;
                                            let ex = (u + t[ax]) as f32;
                                            let got = p[cc][ax];
                                            println!("  ax{ax}: src={:.7} u={:.7} exp={:.7}({:08x}) got={:.7}({:08x}) d={:+}ulp",
                                                s[perm[cc]][ax], u as f32, ex, ex.to_bits(), got, got.to_bits(),
                                                got.to_bits() as i64 - ex.to_bits() as i64);
                                        }
                                    }
                                }
                                break;
