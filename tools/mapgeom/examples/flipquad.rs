//! Dump quad verts for flip faces. Usage: flipquad HIS.ITEM MINE.ITEM SRCFILE
...
fn main() {
    ...
    for t in &r {
        ...
        if let Some(v) = mmap.get(&k) {
            ...
            if dot < 0.0 { flipkeys.insert(k); }
        }
    }
    // source quads producing these keys (via EITHER diagonal!)
    ...
    for (cr, vis, _) in &layers {
        ...
        for f in &cr.faces {
            ...
            // both diagonals' tris
            let mut keys13: Vec<[(i32, i32, i32); 3]> = Vec::new();
            ...
            if keys13.iter().any(|k| flipkeys.contains(k)) && shown < 4 {
                shown += 1;
                println!("quad mat={} verts:", f.material);
                for (i, p) in pts.iter().enumerate() {
                    println!("  v{i}=[{:.5},{:.5},{:.5}]", p[0], p[1], p[2]);
                }
            }
        }
    }
