//! Weld small faces aggressively; angle-rule large faces. Usage: sizeweld SRCFILE
...
fn main() {
    ...
    // NOTE: areas here are FULL-SCALE (unscaled crystal). His areas are 0.25x. Thresholds in full-scale.
    for amin in [0.001f32, 0.01, 0.1, 1.0] {
        for angle in [30.0f32, 44.0] {
            let cos_max = angle.to_radians().cos();
            let mut total_err = 0i64;
            let mut line = format!("amin={amin} deg={angle}:");
            for (stem, want) in &targets {
                ...
                for (_, idxs) in &bypos {
                    ...
                    // seed-largest by area, but faces with area<amin join ANY seed (ignore angle)? 
                    // Interpretation: small faces don't block welding; absorb small faces into nearest seed regardless of angle.
                    ...
                    for gi in ord {
                        ...
                        let ar = corners[gi].2;
                        if ar < amin {
                            // small face: NEVER seeds; joins nearest seed (or seeds if none)
                            if seeds.is_empty() {
                                seeds.push(n);
                            } else {
                                // join nearest (do nothing)
                            }
                        } else if !seeds.iter().any(|s| s[0]*n[0]+s[1]*n[1]+s[2]*n[2] >= cos_max) {
                            seeds.push(n);
                        }
                    }
