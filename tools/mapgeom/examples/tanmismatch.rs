//! Tangent mismatch angle distribution. Usage: tanmismatch FILE SUBSTR
            ...
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                for t in idx.chunks(3) {
                    ...
                    for k in 0..3 {
                        ...
                        let dot = (fu[0]*su[0]+fu[1]*su[1]+fu[2]*su[2]).clamp(-1.0, 1.0);
                        angs.push(dot.acos().to_degrees());
                    }
                }
                angs.sort_by(|x, y| x.partial_cmp(y).unwrap());
                println!("{}: tanU mismatch-angle p50={:.2} p90={:.2} p99={:.2} max={:.1}",
                    a[2], angs[angs.len()/2], angs[angs.len()*9/10], angs[angs.len()*99/100], angs[angs.len()-1]);
                return;
            }
        }
    }
