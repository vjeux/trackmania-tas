//! Greedy cluster solve per his vert. Usage: greedyclus HIS.ITEM MINE.ITEM SUBSTR X Y Z
...
fn main() {
    ...
    // candidate pool: incident + neighbors (cap neighbors at 40)
    let mut pool = inc.clone();
    for j in nbr.iter().take(40) { pool.push(*j); }
    ...
    for (vi, hv) in hisv.iter().enumerate() {
        // greedy: start from incident set, add/remove best until no improvement
        let mut set: BTreeSet<usize> = inc.iter().cloned().collect();
        let mut best = tang(*hv, avg(&set));
        loop {
            let mut improved = false;
            // try removing each
            let cur: Vec<usize> = set.iter().cloned().collect();
            for j in &cur {
                if set.len() <= 1 { break; }
                let mut s2 = set.clone();
                s2.remove(j);
                let d = tang(*hv, avg(&s2));
                if d < best - 0.005 { best = d; set = s2; improved = true; break; }
            }
            if improved { continue; }
            // try adding each pool member
            for j in &pool {
        ...
        let mut sset: Vec<usize> = set.iter().cloned().collect();
        sset.sort();
        println!("his vert{vi} n=({:.4},{:.4},{:.4}) greedy d={best:.3}deg set={sset:?}", hv[0], hv[1], hv[2]);
    }
