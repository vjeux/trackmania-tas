//! Count flipped slivers per material. Usage: flipcount HIS.ITEM MINE.ITEM
...
fn main() {
    ...
    let mut flips: BTreeMap<String, usize> = BTreeMap::new();
    let mut big: BTreeMap<String, usize> = BTreeMap::new(); // >5deg non-flip
    for (t, mat) in &r {
        let mut k = [mk(&t[0]), mk(&t[1]), mk(&t[2])];
        k.sort();
        if let Some(v) = mmap.get(&k) {
            ...
            if ang > 90.0 { *flips.entry(short).or_insert(0) += 1; }
            else if ang > 5.0 { *big.entry(short).or_insert(0) += 1; }
        }
    }
    println!("flips>90deg: {flips:?}");
    println!("5-90deg: {big:?}");
}
