//! Crystal face indices of flipped T2 tris. Usage: flipfaces HIS.ITEM MINE.ITEM SRCFILE
...
fn main() {
    ...
    // source faces (visible) whose T2 key is in flipkeys
    ...
    println!("flip T2 faces ({}): {}", face_ids.len(), face_ids.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
}
