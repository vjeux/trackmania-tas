//! Test R4: flip tri iff dot<0 AND quad-mate dot>0. Compare to actual flips.
//! Usage: r4test HIS.ITEM MINE.ITEM SRCFILE
...
fn main() {
                ...
                if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                    ...
                    for tt in idx.chunks(3) {
                        ...
                        out.push((vec![pos[tt[0] as usize], pos[tt[1] as usize], pos[tt[2] as usize]],
                                  vec![nrm[tt[0] as usize], nrm[tt[1] as usize], nrm[tt[2] as usize]]));
                    }
                }
            }
        }
    ...
    let r = load(&a[1]);
    let m = load(&a[2]);
    // my dots per tri (by index)
    let dot_of = |t: &[[f32; 3]; 3], n: &[[f32; 3]; 3]| -> f32 {
        let fn_ = cross(sub(t[1], t[0]), sub(t[2], t[0]));
        let l = (fn_[0]*fn_[0]+fn_[1]*fn_[1]+fn_[2]*fn_[2]).sqrt();
        if l < 1e-15 { return 2.0; }
        let avg = [(n[0][0]+n[1][0]+n[2][0])/3.0, (n[0][1]+n[1][1]+n[2][1])/3.0, (n[0][2]+n[1][2]+n[2][2])/3.0];
        (fn_[0]*avg[0]+fn_[1]*avg[1]+fn_[2]*avg[2])/l
    };
    // key -> my tri indices (mine may have duplicate keys; take all)
    let mut key2idx: BTreeMap<[(i32, i32, i32); 3], Vec<usize>> = BTreeMap::new();
    for (i, (tt, _)) in m.iter().enumerate() {
        let mut k = [mk(&tt[0]), mk(&tt[1]), mk(&tt[2])];
        k.sort();
        key2idx.entry(k).or_default().push(i);
    }
    // quad mates from source (use t offset for keys)
    ...
    // actual flips (his agrees, mine disagrees) as my-indices
    ...
    let mut actual: BTreeSet<usize> = BTreeSet::new();
    for (tt, _) in &r {
        let mut k = [mk(&tt[0]), mk(&tt[1]), mk(&tt[2])];
        k.sort();
    ...
    println!("actual flips (my-indices): {}", actual.len());
    // R4 prediction on MY tris
    let mut pred: BTreeSet<usize> = BTreeSet::new();
    for (i, (tt, tn)) in m.iter().enumerate() {
        let arr = [[tt[0], tt[1], tt[2]], [tn[0], tn[1], tn[2]]];
        let _ = arr;
        let tarr = [tt[0], tt[1], tt[2]];
        let narr = [tn[0], tn[1], tn[2]];
        if dot_of(&tarr, &narr) >= 0.0 { continue; }
        let mut k = [mk(&tt[0]), mk(&tt[1]), mk(&tt[2])];
        k.sort();
        if let Some(mk2) = mate.get(&k) {
            if let Some(js) = key2idx.get(mk2) {
                // mate dot>0 (any instance)?
                let mut mate_pos = false;
                for j in js {
                    let (u, un) = &m[*j];
                    let uarr = [u[0], u[1], u[2]];
                    let unarr = [un[0], un[1], un[2]];
                    if dot_of(&uarr, &unarr) > 0.0 { mate_pos = true; break; }
                }
                if mate_pos { pred.insert(i); }
            }
        }
    }
    println!("R4 predicted: {}", pred.len());
    let tp = actual.intersection(&pred).count();
    println!("true_positive={tp} false_positive={} false_negative={}", pred.len()-tp, actual.len()-tp);
}
