//! Per-material best theta. Usage: permat_theta SRCFILE
...
fn main() {
    ...
    for (stem, want) in &targets {
        ...
        for deg10 in 1..900 {
            ...
            let err = (clusters as i64 - *want as i64).abs();
            if err < best.0 { best = (err, deg); }
        }
        println!("{stem}: want={want} best_deg={:.1} err={}", best.1, best.0);
    }
}
