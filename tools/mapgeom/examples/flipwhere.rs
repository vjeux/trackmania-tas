//! Locate flipped tris. Usage: flipwhere HIS.ITEM MINE.ITEM
...
fn main() {
    ...
    for path in [&a[1], &a[2]] {
        ...
        println!("{path}: {n} visuals");
    }
    // load both, match, print flipped tri details (his corners, his normals, my normals)
    let load = |path: &str| -> Vec<(Vec<[f32; 3]>, Vec<[f32; 3]>, String)> {
        let data = std::fs::read(path).unwrap();
        let f = mapgeom::static_item::file::parse_file(&data).unwrap();
        let so = f.item.static_object().unwrap();
        let s2 = so.solid2().unwrap();
                ...
                if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                    ...
                    for (d, e) in st.decls.iter().zip(st.elems.iter()) {
                        match e {
                            ...
                            Elem::Word(w) if d.name() == 5 => nrm = w.iter().map(|v| {
                    ...
                    for t in idx.chunks(3) {
                        ...
                        out.push((vec![pos[t[0] as usize], pos[t[1] as usize], pos[t[2] as usize]],
                                  vec![nrm[t[0] as usize], nrm[t[1] as usize], nrm[t[2] as usize]], mat.clone()));
                    }
                }
            }
        }
    ...
    for (t, tn, mat) in &r {
        ...
        if let Some(v) = mmap.get(&k) {
            ...
            if dot < 0.0 && n < 4 {
                n += 1;
                println!("FLIP {} centroid=[{:.4},{:.4},{:.4}] area={:.2e}", mat.rsplit('\\').next().unwrap_or(mat), (t[0][0]+t[1][0]+t[2][0])/3.0, (t[0][1]+t[1][1]+t[2][1])/3.0, (t[0][2]+t[1][2]+t[2][2])/3.0, lr/2.0);
                println!("  his_n=[{:.3},{:.3},{:.3}] my_n=[{:.3},{:.3},{:.3}]", tn[0][0], tn[0][1], tn[0][2], un[perm[0]][0], un[perm[0]][1], un[perm[0]][2]);
            }
        }
    }
}
