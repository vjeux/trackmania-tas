//! Bit-exact test with EXACT source positions (no f32 round-trip).
//! Usage: texact HIS.ITEM SRCFILE
...
fn main() {
    ...
    // src tris: exact src positions (f64) + mmkey of (0.5*src+t approx via d)
    ...
    for (cr, vis, _) in &layers {
        ...
        for f in &cr.faces {
            ...
            // fan (same as face_triangles): (1, i, i+1)
            ...
            for tri in &tris {
                // mmkey via f32 halved+translated (matching key only)
    ...
    let mut miss_ex: Vec<([f64; 3], [f32; 3])> = Vec::new();
                        ...
                        for perm in [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]] {
                            ...
                            if ok {
                                for cc in 0..3 {
                                    for ax in 0..3 {
                                        tot[ax] += 1;
                                        // EXACT: f32(src*0.5 + t) with src exact in f64
                                        let v = (s[perm[cc]][ax]*0.5 + t[ax]) as f32;
                                        if v.to_bits() == p[cc][ax].to_bits() { hit[ax] += 1; }
                                        else if miss_ex.len() < 20000 {
                                            miss_ex.push((s[perm[cc]], p[cc]));
                                        }
                                    }
                                }
                                break;
    ...
    println!("EXACT-src: x={}/{} y={}/{} z={}/{}", hit[0], tot[0], hit[1], tot[1], hit[2], tot[2]);
    // distinct miss verts
    let mut mv: std::collections::BTreeSet<[u32; 3]> = Default::default();
    for (_, h) in &miss_ex {
        mv.insert([h[0].to_bits(), h[1].to_bits(), h[2].to_bits()]);
    }
    println!("distinct miss verts: {}", mv.len());
}
