//! U smoothed within (pos,det-sign) groups? Usage: detgroup HIS.ITEM SUBSTR
    ...
    for g in &s2.shaded_geoms {
        ...
        if let Some(vref) = s2.visuals.get(vi) {
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                // per-face U (face normal GS) + det sign per tri
                // group corners by (pos bits, det sign); average per-face U; compare to stored U.
                let mut groups: BTreeMap<(String, bool), Vec<[f32; 3]>> = BTreeMap::new();
                let mut stored: BTreeMap<(String, bool), Vec<[f32; 3]>> = BTreeMap::new();
                for t in idx.chunks(3) {
                    ...
                    for k in 0..3 {
                        ...
                        groups.entry((pb.clone(), sgn)).or_default().push(fu);
                        stored.entry((pb, sgn)).or_default().push(tu[t[k] as usize]);
                    }
                }
                let (mut agree, mut tot) = (0, 0);
                for (key, us) in &groups {
                    let n = us.len() as f32;
                    let avg = norm([us.iter().map(|x| x[0]).sum::<f32>()/n, us.iter().map(|x| x[1]).sum::<f32>()/n, us.iter().map(|x| x[2]).sum::<f32>()/n]);
                ...
                println!("{}: U avg-within-(pos,det) matches stored {agree}/{tot} ({:.1}%)", a[2], 100.0*agree as f32/tot as f32);
                return;
            }
        }
    }
