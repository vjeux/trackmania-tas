//! Are flipped tris in colliding mmkeys? Usage: flipcoll HIS.ITEM MINE.ITEM
...
fn main() {
    ...
    let (mut flips, mut flips_coll) = (0, 0);
    for t in &r {
        let mut k = [mk(&t[0]), mk(&t[1]), mk(&t[2])];
        k.sort();
        if let Some(v) = mmap.get(&k) {
            ...
            if dot < 0.0 {
                flips += 1;
                if hmap[&k] > 1 || mmap[&k].len() > 1 { flips_coll += 1; }
            }
        }
    }
    println!("flips={flips} in_colliding_keys={flips_coll}");
}
