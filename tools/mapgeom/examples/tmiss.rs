//! Characterize translation misses. Usage: tmiss HIS.ITEM SRCFILE
...
fn main() {
    ...
    // miss signatures per his-vert (bit key): which axes miss
    let mut miss_sig: BTreeMap<[u32; 3], u8> = BTreeMap::new();
    let mut miss_pos: BTreeMap<[u32; 3], [f32; 3]> = BTreeMap::new();
    let mut miss_mat: BTreeMap<[u32; 3], String> = BTreeMap::new();
                            ...
                            if ok {
                                for cc in 0..3 {
                                    ...
                                    for ax in 0..3 {
                                        ...
                                        if ((u + t[ax]) as f32).to_bits() != p[cc][ax].to_bits() { sig |= 1 << ax; }
                                    }
                                    if sig != 0 {
                                        let bk = [p[cc][0].to_bits(), p[cc][1].to_bits(), p[cc][2].to_bits()];
                                        miss_sig.insert(bk, sig);
                                        miss_pos.insert(bk, p[cc]);
                                        miss_mat.insert(bk, short.clone());
                                    }
                                }
                                break;
                            }
    ...
    // signature histogram + worst025 residuals of misses
    let mut hist: BTreeMap<u8, usize> = BTreeMap::new();
    for s in miss_sig.values() { *hist.entry(*s).or_insert(0) += 1; }
    println!("distinct his-verts with >=1 miss: {} sig-hist={hist:?}", miss_sig.len());
    let mut bymat: BTreeMap<String, usize> = BTreeMap::new();
    for (k, m) in &miss_mat { *bymat.entry(m.clone()).or_insert(0) += 1; let _ = k; }
    println!("miss verts by material: {bymat:?}");
    let mut i = 0;
    for (k, p) in &miss_pos {
        if i >= 10 { break; }
        println!("  sig={:03b} {} [{:.5},{:.5},{:.5}]", miss_sig[k], miss_mat[k], p[0], p[1], p[2]);
        i += 1;
    }
}
