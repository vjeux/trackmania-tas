//! U theta plateau. Usage: uplateau HIS.ITEM SUBSTR
    ...
    for g in &s2.shaded_geoms {
        ...
        if let Some(vref) = s2.visuals.get(vi) {
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                // want: |pos+U| (distinct stored U per position)
                ...
                // face-tangents per corner (face normal for GS)
                let mut corners: Vec<([f32; 3], [f32; 3], f32)> = Vec::new(); // (pos, faceU, area)
                for t in idx.chunks(3) {
                    if t.len() < 3 { continue; }
                ...
                print!("{}: want={want} ", a[2]);
                for deg10 in [100, 150, 200, 250, 300] {
