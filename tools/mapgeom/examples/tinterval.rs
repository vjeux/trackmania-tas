//! Interval intersection for exact translation: his_i = round(u_i + t)?
//! Usage: tinterval HIS.ITEM SRCFILE
...
fn main() {
    ...
    // per axis: intersect [his-ulp/2, his+ulp/2] - u over tight pairs
    let mut lo = [f64::NEG_INFINITY; 3];
    let mut hi = [f64::INFINITY; 3];
    let (mut n, mut out) = (0, 0);
                            ...
                            if ok {
                                for cc in 0..3 {
                                    ...
                                    for ax in 0..3 {
                                        ...
                                        let h = p[cc][ax];
                                        // his must equal round(u+t): t in [h-u-ulp/2, h-u+ulp/2]
                                        // ulp of values near h:
                                        let ulp = 2f64.powi((h.abs().max(1e-30).log2().floor()) as i32 - 23);
                                        let dh = h as f64 - u as f64;
                                        lo[ax] = lo[ax].max(dh - ulp/2.0);
                                        hi[ax] = hi[ax].min(dh + ulp/2.0);
                                        if lo[ax] > hi[ax] { out += 1; }
                                    }
                                }
                                break;
                            }
    ...
    println!("n={n} contradictions={out}");
    for ax in 0..3 {
        println!("ax{ax}: t in [{:.10},{:.10}] width={:.3e}", lo[ax], hi[ax], (hi[ax]-lo[ax]).max(0.0));
    }
}
