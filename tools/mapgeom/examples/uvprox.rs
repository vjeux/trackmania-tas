//! Pool faces with uv-centroid within EPS of spot uv. Usage: uvprox HIS.ITEM MINE.ITEM SUBSTR X Y Z EPS
...
fn main() {
    ...
    let eps: f32 = a[7].parse().unwrap();
    let data = std::fs::read(&a[2]).unwrap();
    let f = mapgeom::static_item::file::parse_file(&data).unwrap();
    let so = f.item.static_object().unwrap();
    let s2 = so.solid2().unwrap();
    // all tris with (face normal, uv centroid)
    let mut faces: Vec<([f32;3],[f32;2])> = Vec::new();
    // spot uv (from mine)
    let mut spot_uv = [0.0; 2];
        ...
        if let Some(vref) = s2.visuals.get(vi) {
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                for t in idx.chunks(3) {
                    ...
                    let cen = [(us[0][0]+us[1][0]+us[2][0])/3.0, (us[0][1]+us[1][1]+us[2][1])/3.0];
                    faces.push(([cr[0]/l, cr[1]/l, cr[2]/l], cen));
                    for k in 0..3 {
                        let d = ((ps[k][0]-q[0]).powi(2)+(ps[k][1]-q[1]).powi(2)+(ps[k][2]-q[2]).powi(2)).sqrt();
                        if d < 1e-6 { spot_uv = us[k]; }
                    }
                }
            }
        }
    ...
    let mut c = 0;
    for (fn_, cen) in &faces {
        let d = ((cen[0]-spot_uv[0]).powi(2)+(cen[1]-spot_uv[1]).powi(2)).sqrt();
        if d < eps { for i in 0..3 { acc[i] += fn_[i] as f64; } c += 1; }
    }
    ...
    println!("eps={eps} npool={c} avg=({:.4},{:.4},{:.4}) his=({:.4},{:.4},{:.4}) d={:.3}deg", avg[0], avg[1], avg[2], his[0], his[1], his[2], ang(his, avg));
