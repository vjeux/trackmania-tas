//! Are flips T1 or T2 of quad? Usage: flipt12 HIS.ITEM MINE.ITEM SRCFILE
...
fn main() {
    ...
    for (cr, vis, _) in &layers {
        ...
        for f in &cr.faces {
            ...
            if flipkeys.contains(&ks[0]) { t1flip += 1; }
            if flipkeys.contains(&ks[1]) { t2flip += 1; }
        }
    }
    println!("flips: T1={t1flip} T2={t2flip}");
}
