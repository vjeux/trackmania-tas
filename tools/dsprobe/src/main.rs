...
fn main() {
    ...
    if std::env::args().any(|x| x == "--help" || x == "-h") {
        ...
        print!("{}", r#"
dsprobe -- probe a dedicated-server endpoint through the forward proxy

  dsprobe HOST PORT [PATH]
        Open a connection and report what answers. Used to check a server is
        reachable before a run blames the engine for a network failure.
"#);
        std::process::exit(0);
    }
