...
/// Measured on the 25 Summer sources, the tiny bakes and the giant editor bakes
/// (coordinator + giant child, 2026-09-22/23):
///
/// ```text
/// base = P + N_authored + (S_x·S_z − replaced) + G
/// ```
///
/// * P = 16384 on the Stadium decorations (48x48Screen155*, NoStadium48x48*;
///   objects 0..3 are the decoration's own charts on Screen155, none on
///   NoStadium), 0 on the terrain collections;
/// * N_authored = the unbaked blocks (free ones included; embedded
///   `…_CustomBlock`s are none — the tiny 05's 46 free water tiles add nothing);
/// * S_x·S_z ground tiles, generated by the game for the map grid (baked records
///   in the file do NOT count — 01 ×2 carries 1886 Sea records and its base is
///   128² exactly); on a terrain collection an authored block replaces its
///   column's tile (17 ×2, 04 ×2: one kept block → S² exactly), on Stadium it does
///   not (25 ×2: 16384 + 1 + 96² + 7);
/// * G = the other generated pieces: 432 apron pieces of the Screen155 stadium
///   (the empty tiny 05: 2736 = 2304 + 432), the pillars/walls the game grows
///   under elevated Stadium blocks (7 under 25 ×2's kept platform, 2108 around
///   05 ×2's 604 pool tiles) — map-specific, not predicted here: `--base-extra N`
///   (or an editor bake) for maps with such blocks.
///
/// A Nadeo map's baked chunk holds exactly the game's generated set, so there
/// base = P + unbaked + baked (25/25 sources); `basecheck` prints both.
pub struct BaseRule {
    pub deco_const: u32,
    pub authored: u32,
...
/// `authored`: (grid cell x, grid cell z, name) of every UNBAKED block.
pub fn base_rule<'a>(envir: &str, decoration: &str, size: [i32; 3], authored: impl Iterator<Item = (i32, i32, &'a str)>, extra: u32) -> BaseRule {
    ...
    let apron = if stadium && !d.starts_with("nostadium") { 432 } else { 0 };
    BaseRule { deco_const: if stadium { 16384 } else { 0 }, authored: n_auth, custom_blocks: n_custom, ground_cols, replaced, extra: apron + extra, stadium }
}
