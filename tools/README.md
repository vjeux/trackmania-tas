...
### `C-route` — the only check that does not read the record

Added 2026-08-21, after a file passed **every other check in this gate** while
carrying somebody else's run.

On 227654 `fk regen` wrote the CONTAINER's trajectory instead of ours, and:
`B-contam` passed (0.000511 m from ailiei.'s record is the client-vs-server
floor, not bit-identical) · `C-spawn` passed (his run starts at the same spawn)
· `C-oracle` passed (the oracle reads the TAPE, and the tape is ours) ·
`E-stale` passed (two independent generations agreed exactly). They agreed with
each other because **they were all reading the same poisoned source.**

What convicted it was a different instrument: `fk btraj2` re-simulates the same
tape and dumps the car's position per tick without going near the record. Our
car was 0.7249 m mean / 1.8607 m max from the line the record claimed. Two of
our tapes differing at race 11.270 s — inside the recorded window — had produced
**bit-identical** records, which two different simulations cannot do.
...
**No `--route` is a REFUSAL, not an `n/a`** — a file that cannot be
route-checked is exactly the file this hole hides in. A route that shares no
instant with the record is `UNMEASURED` (exit 3), which is neither clean nor
convicted.

The bar is derived from the instrument: the route CSV carries six significant
digits, so its own quantum is `2e-6 x |coordinate|` (~3 mm at 1400), and the bar
is twenty of those. Measured: clean files 0.0005 m, the poisoned one 0.5479 m,
a deliberately mismatched route 783 m. Nothing lands in the gap.

**The transferable lesson is not the bug.** Every check agreed, and they were
all wrong together, because they shared a source. `C-route` works because it
comes from a different instrument — not because it is a better test.

And the corollary, which invalidates a reassurance this project leaned on:
**distance from a human recording proves only that the writer did not follow
THAT human.** A poisoned file whose container is our own earlier run sits zero
metres from nobody and passes contamination against everyone.
