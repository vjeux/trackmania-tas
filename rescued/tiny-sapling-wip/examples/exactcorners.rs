//! Tri corners bit-exact at a position. Usage: exactcorners FILE SUBSTR X Y Z
        ...
        if let Some(vref) = s2.visuals.get(vi) {
            if let Some(mapgeom::static_item::Node::Visual(vis)) = vref.inline.as_deref() {
                ...
                for (ti, t) in idx.chunks(3).enumerate() {
                    ...
                    for k in 0..3 {
                        if [ps[k][0].to_bits(), ps[k][1].to_bits(), ps[k][2].to_bits()] == qb {
                            ...
                            println!("tri{ti} corner{k} fn=({:.4},{:.4},{:.4}) area={:.6}", cr[0]/l, cr[1]/l, cr[2]/l, l/2.0);
                        }
                    }
                }
            }
