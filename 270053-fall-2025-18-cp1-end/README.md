...
https://github.com/user-attachments/assets/13c4e128-eddd-447d-8c3d-ddd5c859871e

**Both runs are in this clip, and you cannot tell them apart. That is the
finding.** Our tape and AffiTM's world record are on screen together for the
whole four and a half seconds, and they never separate by more than **0.48 m** —
mean **0.08 m** — which is a fraction of a car length, so the two cars render
*inside each other*. The green-and-yellow bodywork flickering through the
magenta is the second car; there is no moment where a viewer sees two vehicles.
Six players are tied at 4.495 and the entire top 15 fits inside 0.003, and this
is what that looks like from the driver's seat: one racing line, driven to the
half-metre, by everybody.

The 0.003 we are ahead by is **14 cm at the line** — about a tenth of one frame
of this video. **A chase camera cannot show it, and this clip does not pretend
to.** What the clip does show is where the run is *not* decided: the kink, the
straight and the sweeper are common property, driven identically by a machine
searching for the author medal and by the best human on the board.

The camera is bolted to AffiTM's car rather than ours, which is why the shot
stays alive through the finish: our tape's samples stop at 4.45 s, 42 ms before
our own crossing, so a camera following our car goes blind before the flag.
Opponent provenance: **rank 1 on the map's live leaderboard**, downloaded as
recorded (`rank001_4495_AffiTM.Ghost.Gbx`, md5 `1d5a507f32123fb3904e3330ac1b8190`),
never regenerated by us — the game reads the name *AffiTM* off the file itself.
