...
| **Fall 2022 staged build / public October update** | September 30 and October 6 are one historical build; October 1 is only a release date. V5 proved crash-free but covered only 13% of the Sep. 30 physics closure and bound 12 thunks to changed/missing current helpers. The earlier three-way comparison used the correct cross-version `Loc.translation` mirror (`+0x540` Sep. 30, `+0x538` current), but it is quantized to `0.001 m`; its multi-metre mismatch remains evidence while the 1 mm rung does not. Static pack decryption now isolates the historical difference to CarSport vehicle data, and an unquantized data-level control is in progress. Fall remains fail-closed. |
