...
| **Fall 2022 staged build / public October update** | September 30 and October 6 are one historical build; October 1 is only a release date. V5 proved crash-free but targeted code, while later controls refuted both V5 and a complete Sep. 30 `VehicleTunings` transplant. Correct elapsed-time alignment shows the two exact clients are bit-identical for `1.360 s`, then differ by one ULP in full-precision Z (`0x438fac31` vs `0x438fac32`, `0.00003052 m`). Earlier metre-scale figures were amplified open-loop descendants and are withdrawn as force-law measurements. The responsible state predicate or other asset is still being localized; Fall remains fail-closed. |
