...
An Openplanet Developer-mode plugin exposing every behaviorally confirmed official profile in the complete inventory: four Stadium epochs, four Snow epochs, three Rally epochs, and one Desert epoch. It deliberately does not turn static handler or pack hashes into fake user-facing “engines”: implementation fingerprints are not behavioral proof.
...
Snow release and January patch the already-loaded `CPlugSurface` in memory. The plugin requires the exact seven-shape compound, verifies the installed first three ellipsoids and four unchanged `0.47` wheel spheres, then changes only the first three type/radius fields to the historical spheres. It retains the exact current scales and restores them on profile switch or unload.
...
Snow, Rally, and Desert are separate official vehicle families, and the selector exposes only behavior changes confirmed by Nadeo's release notes:
...
The plugin does not infer additional epochs from executable or pack hashes. There are no era-matched Snow/Rally/Desert replay controls; official changelog statements establish these boundaries. `evidence/vehicle-family-behavior-changes.md` carries the source links and interpretation.

The three Rally profiles are executable on build 128130 as two independent, transactional axes:
...
`payloads/manifest.tsv` records the byte-level provenance. The release helper validates the exact executable and payload, backs up any existing loose item, writes atomically, verifies readback, and refuses foreign files. Run `bin/rally_item_overlay-linux-x86_64 install <Trackmania-directory>` before Rally release, and `restore` followed by a restart before selecting April/current.
...
3. Select one of the confirmed official profiles.
4. For any historical memory-backed profile, arm **Experimental native profiles**, then apply.
5. Use offline maps only.
6. Return to the main menu and select the current profile for that family to restore the installed game's original code and loaded data.
...
The public full-client archive has no snapshot in the exact `2022-03-29..2022-09-20` interval. Certification therefore requires an external/private April–August 2022 full-client backup. This is an archive-coverage gap, not a claim that no matching client existed. Imported ghost playback is not accepted because it may display recorded samples rather than re-simulate inputs.
