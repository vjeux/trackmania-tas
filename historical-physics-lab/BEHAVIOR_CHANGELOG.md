...
| Staged 2022-09-21 / public 2022-10-01 (Fall 2022) | Sep. 30 and Oct. 6 share the canonical digest, so October 1 is not the executable boundary. Static work proves V5 covered only 13% of the historical closure and that `VehicleTunings` changed shape, but every client trajectory collected so far used inherited object offsets for tape input. The actual input axes are frame-local, so those writes corrupted unrelated car fields and all resulting divergence figures are withdrawn. | No client-side behavior claim is currently admissible. The next control uses only witnessed OS-native input, with two fresh-process floors, before localizing code or data. Fall remains fail-closed. |
