...
| Staged 2022-09-21 / public 2022-10-01 (Fall 2022) | Sep. 30 and Oct. 6 share the canonical digest, so October 1 is not the executable boundary. Correct alignment by elapsed tape time—not raw `sim_ms`, whose origins differ—shows a `1.360 s` bit-identical prefix. The first difference is one ULP in full-precision Z: `0x438fac31` vs `0x438fac32` (`0.00003052 m`). A complete Sep. 30 `VehicleTunings` pack is accepted and read by current, but produces a bit-identical stock trajectory; a deliberate scalar perturbation moves the car `237.583 m`, proving that null is meaningful. | V5 and the CarSport `VehicleTunings` hypothesis are both rejected. Earlier metre-scale errors were open-loop amplification from the one-ULP fork and are withdrawn as direct force-law measurements. The responsible ground/air-state predicate, other code path, or other asset is unresolved; Fall remains fail-closed. |
