...
https://github.com/user-attachments/assets/dd84557f-35fc-4296-9f5a-89b3cb6731ff
...
**[untitled 02](276877-untitled-02)** — author time `31.399` — **not route-enforced**, there are no checkpoints and the only thing you must touch is the finish gate · **beaten by a human: 14.959 (Its_Cam.)**, who took the same shortcut we did and reached the same gate at 2.583 · ours **8.898** (the 8.898 ghost was withdrawn — it carried a stranger's record — so the fastest file published there is 9.415) · 1 record
...
that decides which, by asking whether any file in the map's `replays/` states
the number on this page.

**"Best human"** is the online world record as of the capture at the top. A
margin printed on a map's own page was measured against whatever record stood
when it was written; where that record has since moved, the page keeps the old
one in the column heading rather than re-basing the numbers, because re-basing
them would be arithmetic dressed as measurement.

**Fewest inputs.** Several maps here fall to a handful of key presses — 12 on
Get in the Hole, 14 in the flight on YEET Fall 2024 - 04, 4 in the climb on
MAX-UP, 19 on Fall 2025 - 13 — and *fewest is not easiest*: on several maps the
...
`identical-tapes / EXPECTED-SAME-INPUTS`, which is to say clean. Its 16
`REFUSE-ONE-RUN-TWICE` pairs are **not adjudicated** — settling one needs
re-simulation against the map — so no page here calls them duplicates.
